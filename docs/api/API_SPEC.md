...
כל שורת `GET /api/reports/cases` כוללת `id` יציב לפתיחת פרטי הקריאה מתוך תפריט הפעולות;
`case_number` נשאר הערך העסקי המלא בחוזה וה־UI מסיר ממנו את הקידומת הטכנית בתצוגה בלבד.
...
מרכז העבודה תומך בפרמטר `search` לחיפוש חופשי לפי מספר קריאה או נושא, בנוסף למסנן
`title` הממוקד. `sort` נשלח בפורמט `field:direction`, ו־`page`/`page_size` שולטים
בפייג'ינציה בצד השרת. כל האפשרויות ממשיכות לפעול לאחר סינון הנראות המשותף.
