...
def test_workflow_crud_and_sla_specificity() -> None:
    ...
    request_type = client.get(
        f"/api/request-types?environment_id={environment['id']}", headers=auth
    ).json()[0]
    priority = client.get(
        f"/api/environments/{environment['id']}/priorities", headers=auth
    ).json()[0]
...
def test_valid_and_invalid_workflow_transition_and_notification() -> None:
    ...
    with SessionLocal() as db:
        ...
        priority = db.scalar(
            select(PriorityDefinition).where(PriorityDefinition.environment_id == env.id)
        )
    ...
    legal = client.post(
        f"/api/cases/{case_id}/workflow-transitions/{transition_id}", headers=auth, json={}
    )
