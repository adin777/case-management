...
class CaseVisibilityService:
    ...
    def managed_environment_ids(self) -> list[uuid.UUID]:
        if self.user.is_system_admin:
            return list(self.db.scalars(select(Environment.id)))
        return list(self.db.scalars(select(EnvironmentMembership.environment_id).where(
            EnvironmentMembership.user_id == self.user.id,
            EnvironmentMembership.is_active.is_(True),
            EnvironmentMembership.is_environment_manager.is_(True),
        )))

    def permission_readable_environment_ids(self) -> list[uuid.UUID]:
        if self.user.is_system_admin:
            return list(self.db.scalars(select(Environment.id)))
        result = []
    ...
    def apply(self, query: Select, *, include_participants: bool = True) -> Select:
        if self.user.is_system_admin:
            return query
        conditions = [
            Case.environment_id.in_(self.managed_environment_ids()),
            ...
            Case.environment_id.in_(self.permission_readable_environment_ids()),
        ]
