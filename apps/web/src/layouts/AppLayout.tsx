...
import { Add, Approval, Assessment, Dashboard, Groups, LockPerson, Logout, Menu, Notifications, Settings, SwitchAccount, Tune } from '@mui/icons-material';
...
type LinkItem = { url: string; label: string; icon: ReactNode; admin?: boolean; implementer?: boolean };
export function AppLayout() {
  ...
  const canImplement=user?.is_system_admin||Boolean((user as User&{can_implement?:boolean}|undefined)?.can_implement);
  const visibleLinks = links.filter((link) => (!link.admin || user?.is_system_admin)&&(!link.implementer||canImplement));
