...
export function DashboardPage() {
  const [searchParams, setSearchParams] = useSearchParams(); const view = searchParams.get('tab') === 'assigned' ? 'assigned' : 'my'; const queue=searchParams.get('queue')||'';
  const [filters, setFilters] = useState(initial); const [page, setPage] = useState(1); const [pageSize, setPageSize] = useState(25); const [sort, setSort] = useState('updated_at:desc'); const [visibleColumns,setVisibleColumns]=useState<string[]>([...workspaceColumns]); const[selectedIds,setSelectedIds]=useState<string[]>([]);const[previewId,setPreviewId]=useState<string>();
