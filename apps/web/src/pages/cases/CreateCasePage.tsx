...
export function CreateCasePage() {
  ...
  async function submit(event: FormEvent) {
    ...
    if (!environmentId || !requestTypeId || !title.trim() || !description.trim() || !priorityId) { setError('יש למלא את כל שדות הליבה'); return; }
    setSubmitting(true);
    try {
      const result = await api<Case>('/cases', { method: 'POST', body: JSON.stringify({
        ...
        priority: selectedPriority?.code || 'normal', priority_id: priorityId,
  ...
  return <Container maxWidth="md"><Stack spacing={3}>
    ...
    <Card variant="outlined"><CardContent><Stack component="form" onSubmit={submit} spacing={2.5}>
      ...
      <Button type="submit" variant="contained" size="large" disabled={!form || submitting}>{submitting ? <><CircularProgress size={20} color="inherit" sx={{ ml: 1 }} />שומר קריאה...</> : 'פתיחת הקריאה'}</Button>
