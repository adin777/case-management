      ...
      <Grid size={{ xs: 12, lg: 8 }}><Stack spacing={2.5}>
        <Box ref={detailsRef}><CaseSection title="פרטי הקריאה" icon={<DescriptionOutlined/>}><Stack spacing={2.25}>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, minmax(0, 1fr))' }, gap: 2 }}>
            ...
            <Card variant="outlined" sx={{ p: 1.5, bgcolor: 'background.default', boxShadow: 'none' }}><CardContent sx={{ p: '0!important' }}><Typography variant="caption" color="text.secondary">פותח הקריאה</Typography><Typography fontWeight={650}>{item.reporter_name || 'לא זמין'}</Typography><Typography variant="body2" color="text.secondary">{item.reporter_email || ''}</Typography></CardContent></Card>
            <Card variant="outlined" sx={{ p: 1.5, bgcolor: 'background.default', boxShadow: 'none' }}><CardContent sx={{ p: '0!important' }}><Typography variant="caption" color="text.secondary">סביבה</Typography><Typography fontWeight={650}>{item.environment_name || 'לא זמינה'}</Typography></CardContent></Card>
          </Box>
