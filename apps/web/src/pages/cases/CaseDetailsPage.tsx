...
import type { Case, Comment, Environment, Form, Participant, RequestType, User } from '../../types';
...
function displayValue(value: Case['values'][number] | undefined) {
  ...
  if (resolved && typeof resolved === 'object') return 'מידע נוסף נשמר';
  ...
  return <Container maxWidth="xl"><Stack spacing={2}>
    ...
    <Grid container spacing={3} alignItems="flex-start">
      <Grid size={{ xs: 12, md: 7 }}><Stack spacing={2}>
        ...
        <Card variant="outlined"><CardContent><Typography variant="h6" fontWeight={800}>מידע נוסף</Typography><Divider sx={{ my: 1.5 }} />{form?.fields.length ? form.fields.map((field) => <Box key={field.id} sx={{ py: 1 }}><Typography variant="caption" color="text.secondary">{field.label_he}</Typography><Typography>{displayValue(item.values.find((value) => value.field_definition_id === field.id))}</Typography></Box>) : <Typography color="text.secondary">אין שדות נוספים</Typography>}</CardContent></Card>
      </Stack></Grid>
