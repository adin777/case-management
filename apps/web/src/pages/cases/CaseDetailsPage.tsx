...
import {
  ...
  Dialog, DialogActions, DialogContent, DialogTitle, Divider, FormControl, Grid, InputLabel, MenuItem, Paper, Select, Stack, Tab, Tabs,
  TextField, Typography,
} from '@mui/material';
import { api } from '../../api/client';
import type { Case, Comment, Form, Participant, Priority, RequestType, SubPriority, User } from '../../types';
...
export function CaseDetailsPage() {
  ...
  const [decisionTask, setDecisionTask] = useState<{ id: string; decision: 'approved' | 'rejected' | 'returned' }>();
  ...
  const { data: subPriorities = [] } = useQuery({ queryKey: ['sub-priorities', item?.environment_id], queryFn: () => api<SubPriority[]>(`/environments/${item!.environment_id}/sub-priorities`), enabled: !!item });
  ...
  const { data: allowed = [] } = useQuery({ queryKey: ['transitions', id, item?.workflow_status_id], queryFn: () => api<{ id: string; label_he: string }[]>(`/cases/${id}/allowed-transitions`), enabled: !!item?.permissions.can_change_status });
  const { data: caseConfig } = useQuery({ queryKey: ['case-config', item?.request_type_id], queryFn: () => api<{ statuses: { id: string; label_he: string }[] }>(`/request-types/${item!.request_type_id}/case-config`), enabled: !!item });
  ...
  const priority = priorities.find((row) => row.id === item?.priority_id);
  const subPriority = subPriorities.find((row) => row.id === item?.sub_priority_id);
  ...
  async function transition(workflow_status_id: string) {
    try { await api(`/cases/${id}/transitions`, { method: 'POST', body: JSON.stringify({ workflow_status_id }) }); refreshCase(); }
  ...
  async function decide() { if (!decisionTask) return; try { await api(`/approval-tasks/${decisionTask.id}/decision`, { method: 'POST', body: JSON.stringify({ decision: decisionTask.decision, comment: decisionComment || null }) }); setDecisionTask(undefined); setDecisionComment(''); setSuccess('החלטת האישור נשמרה'); await Promise.all([client.invalidateQueries({ queryKey: ['approvals', id] }), refreshCase()]); } catch (caught) { setError((caught as Error).message); } }
  async function saveCase(payload: object) { setError(''); try { await api(`/cases/${id}`, { method: 'PATCH', body: JSON.stringify(payload) }); setEditOpen(false); setSuccess('פרטי הקריאה עודכנו'); await refreshCase(); } catch (caught) { setError((caught as Error).message); } }
  ...
  return <Container maxWidth="xl"><Stack spacing={2}>
    ...
    <Box><Typography color="primary" fontWeight={800}>{item.case_number}</Typography><Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" gap={1}><Typography variant="h4" fontWeight={800}>{item.title}</Typography><Stack direction="row" gap={1} flexWrap="wrap"><Chip label={caseConfig?.statuses.find((status) => status.id === item.workflow_status_id)?.label_he || statusLabel(item.status)} color="primary"/>{item.is_approved && <Chip color="success" label={`מאושר${item.approved_by_summary ? ` · ${item.approved_by_summary}` : ''}`} />}{item.approval_status === 'pending' && <Chip color="info" label="ממתינה לאישור" />}{item.is_locked && <Chip icon={<Lock/>} color="warning" label="נעולה לשינויים"/>}{item.permissions.can_edit && <Button startIcon={<Edit/>} variant="outlined" onClick={() => setEditOpen(true)}>עריכת קריאה</Button>}{item.permissions.can_lock && <Button startIcon={item.is_locked ? <LockOpen/> : <Lock/>} color="warning" variant="outlined" onClick={() => setLockOpen(true)}>{item.is_locked ? 'פתיחת נעילה' : 'נעילת קריאה'}</Button>}</Stack></Stack>{item.is_locked && <Alert severity="warning" sx={{ mt: 1 }}>הקריאה נעולה לשינויים{item.lock_reason ? `: ${item.lock_reason}` : ''}. ניתן עדיין להוסיף תגובות.</Alert>}</Box>
    <Grid container spacing={3} alignItems="flex-start">
      <Grid size={{ xs: 12, md: 7 }}><Stack spacing={2}>
        <Card variant="outlined"><CardContent><Typography variant="h6" fontWeight={800}>פרטי הקריאה</Typography><Divider sx={{ my: 1.5 }} /><Typography variant="caption" color="text.secondary">תיאור</Typography><Typography sx={{ whiteSpace: 'pre-wrap', mb: 2 }}>{item.description}</Typography><Grid container spacing={2}><Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption">סוג קריאה</Typography><Typography fontWeight={600}>{requestType?.name_he || '—'}</Typography></Grid><Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption">סטטוס</Typography><Typography fontWeight={600}>{caseConfig?.statuses.find((status) => status.id === item.workflow_status_id)?.label_he || statusLabel(item.status)}</Typography></Grid><Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption">עדיפות</Typography><Typography>{priority?.label_he || item.priority}</Typography></Grid><Grid size={{ xs: 12, sm: 6 }}><Typography variant="caption">תת-עדיפות</Typography><Typography>{subPriority?.label_he || 'ללא'}</Typography></Grid>{form?.fields.filter((field) => field.is_active !== false).map((field) => <Grid key={field.id} size={{ xs: 12, sm: 6 }}><Typography variant="caption">{field.label_he}</Typography><Typography>{displayValue(item.values.find((value) => value.field_definition_id === field.id))}</Typography></Grid>)}{item.permissions.can_change_status && <Grid size={{ xs: 12, sm: 6 }}><FormControl fullWidth disabled={!allowed.length}><InputLabel>שינוי סטטוס</InputLabel><Select label="שינוי סטטוס" value="" onChange={(event) => transition(event.target.value)}>{allowed.map((status) => <MenuItem key={status.id} value={status.id}>{status.label_he}</MenuItem>)}</Select></FormControl></Grid>}{item.permissions.can_assign && <Grid size={{ xs: 12, sm: 6 }}><FormControl fullWidth><InputLabel>הקצאת מטפל</InputLabel><Select label="הקצאת מטפל" value={users.some((user) => user.id === item.assignee_id) ? item.assignee_id : ''} onChange={(event) => assign(event.target.value)}><MenuItem value="">ללא מטפל</MenuItem>{users.map((user) => <MenuItem key={user.id} value={user.id}>{user.display_name}</MenuItem>)}</Select></FormControl></Grid>}</Grid></CardContent></Card>
    ...
    <CaseEditDialog open={editOpen} item={item} form={form} priorities={priorities} subPriorities={subPriorities} users={users} onClose={() => setEditOpen(false)} onSave={saveCase}/><CaseLockDialog open={lockOpen} locked={item.is_locked} onClose={() => setLockOpen(false)} onSave={saveLock}/>
    <Dialog open={!!decisionTask} onClose={() => setDecisionTask(undefined)} fullWidth maxWidth="sm"><DialogTitle>{{ approved: 'אישור הבקשה', rejected: 'דחיית הבקשה', returned: 'החזרת הבקשה לתיקון' }[decisionTask?.decision || 'approved']}</DialogTitle><DialogContent><Stack spacing={2} sx={{ mt: 1 }}><Alert severity={decisionTask?.decision === 'approved' ? 'success' : 'warning'}>{decisionTask?.decision === 'approved' ? 'האישור יקדם את הסבב לשלב הבא כאשר יושלם מספר האישורים הנדרש.' : 'נדרשת הנמקה שתישמר ביומן הביקורת.'}</Alert><TextField autoFocus multiline minRows={3} label={decisionTask?.decision === 'approved' ? 'הערה (אופציונלי)' : 'נימוק (חובה)'} value={decisionComment} onChange={(event) => setDecisionComment(event.target.value)} /></Stack></DialogContent><DialogActions><Button onClick={() => setDecisionTask(undefined)}>ביטול</Button><Button variant="contained" color={decisionTask?.decision === 'rejected' ? 'error' : 'primary'} disabled={decisionTask?.decision !== 'approved' && !decisionComment.trim()} onClick={decide}>אישור החלטה</Button></DialogActions></Dialog>
  </Stack></Container>;
}
