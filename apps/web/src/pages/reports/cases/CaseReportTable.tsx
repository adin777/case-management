...
import { Card, CardContent, IconButton, Menu, MenuItem, Paper, Stack, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TableSortLabel, Typography } from '@mui/material';
...
export function CaseReportTable({ rows, visible, filters, onFilters }: { rows: CaseReportRow[]; visible: (keyof CaseReportRow)[]; filters: ReportFilters; onFilters: (value: ReportFilters) => void; sources: unknown }) {
  const { t, i18n } = useTranslation(); const navigate = useNavigate(); const [menu, setMenu] = useState<{ anchor: HTMLElement; id: string }>();
