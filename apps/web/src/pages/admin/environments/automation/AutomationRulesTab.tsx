...
export function AutomationRulesTab({ environment }: { environment: Environment }) {
  ...
  return <Stack spacing={2}>{rules.map((rule) => <Paper key={rule.id} variant="outlined" sx={{ p: 2 }}><Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between"><span><Typography fontWeight={650}>{rule.name}</Typography><Typography variant="body2" color="text.secondary">{rule.is_active ? 'פעילה' : 'מושבתת'} · סדר {rule.priority}</Typography></span><Button onClick={() => setEditing(rule)}>עריכה</Button></Stack></Paper>)}<AutomationRuleEditor environment={environment} fields={fields} rule={editing} onSave={save} onCancel={() => setEditing(undefined)}/></Stack>;
}
