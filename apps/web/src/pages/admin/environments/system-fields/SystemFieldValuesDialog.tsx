...
export function SystemFieldValuesDialog({ environment, field, onClose }: { environment: Environment; field?: SystemField; onClose: () => void }) {
  ...
  return <Dialog open={!!field} onClose={onClose} fullWidth maxWidth="md"><DialogTitle>ניהול ערכי {field?.label_he}</DialogTitle><DialogContent><Stack spacing={2} sx={{ mt: 1 }}>{error && <Alert severity="error">{error}</Alert>}
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={reorder}><SortableContext items={ordered.map(item=>item.id)} strategy={verticalListSortingStrategy}>{ordered.map((item) => <SortableValue key={item.id} item={item}><Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'center' }}><span><Typography fontWeight={800}>{item.label_he}</Typography><Typography variant="body2" color="text.secondary">{item.is_active === false ? 'מושבת' : 'פעיל'}</Typography></span><Button onClick={() => choose(item)}>עריכה</Button></Stack></SortableValue>)}</SortableContext></DndContext>
