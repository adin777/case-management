...
import { EnvironmentUserFieldsTab } from './tabs/EnvironmentUserFieldsTab';

const labels = ['כללי', 'שדות מערכת', 'שדות נוספים', 'שדות משתמש בסביבה', 'משתמשים', 'שיוך אוטומטי', 'אוטומציות', 'אישורים', 'SLA', 'ניהול ידע'];
...
export function EnvironmentsPage() {
  ...
  return <Container maxWidth="xl"><Stack spacing={3}>
    ...
    {selected && <Paper variant="outlined"><Tabs value={tab} onChange={(_, value) => setTab(value)} variant="scrollable" scrollButtons="auto">{labels.map((label) => <Tab key={label} label={label}/>)}</Tabs><Box sx={{ p: { xs: 2, md: 3 } }}><Stack spacing={2}>
      <Box><Button variant="outlined" onClick={() => { setCloneForm({ ...empty, name_he: `${selected.name_he} - עותק`, name_en: `${selected.name_en} copy`, copy_memberships: false, copy_knowledge: false }); setCloneOpen(true); }}>שכפול סביבה</Button></Box>
      {tab === 0 && <EnvironmentGeneralTab environment={selected} onSaved={() => client.invalidateQueries({ queryKey: ['environments'] })}/>} {tab === 1 && <SystemFieldsTab environment={selected}/>} {tab === 2 && <CaseFieldsTab environment={selected}/>} {tab === 3 && <EnvironmentUserFieldsTab environment={selected}/>} {tab === 4 && <EnvironmentAccessTab environment={selected}/>} {tab === 5 && <EnvironmentAssignmentRulesTab environment={selected}/>} {tab === 6 && <AutomationRulesTab environment={selected}/>} {tab === 7 && <ApprovalFlowsTab environment={selected}/>} {tab === 8 && <SlaPoliciesTab environment={selected}/>} {tab === 9 && <KnowledgeTab environment={selected}/>} </Stack></Box></Paper>}
  </Stack>
