...
export function EnvironmentAccessTab({environment}:{environment:Environment}) {
  ...
  return <Stack spacing={2}><Typography variant="h6">משתמשים והרשאות סביבה</Typography><Tabs value={mode} onChange={(_,next)=>{setMode(next);setValue('')}}><Tab label="משתמש"/><Tab label="קבוצה"/></Tabs>{mode===0&&<FormControlLabel control={<Switch checked={activeOnly} onChange={e=>setActiveOnly(e.target.checked)}/>} label="משתמשים פעילים בלבד"/>}{members.map(m=><Paper key={m.id} variant="outlined" sx={{p:2}}><Stack direction={{xs:'column',sm:'row'}} gap={1} alignItems={{sm:'center'}}><Typography fontWeight={800} sx={{flex:1}}>{m.user_name||m.group_name}</Typography><Chip size="small" label={m.group_id?'קבוצה':m.source==='rule'?'כלל אוטומטי':'משתמש ישיר'}/>{m.user_id&&<FormControlLabel control={<Switch checked={m.is_environment_manager} onChange={e=>manager(m,e.target.checked)}/>} label="מנהל סביבה"/>}<Button color="error" disabled={m.source==='rule'} onClick={()=>remove(m.id)}>הסרה</Button></Stack></Paper>)}<Paper variant="outlined" sx={{p:2}}><Stack direction={{xs:'column',sm:'row'}} gap={1}><TextField select fullWidth label={mode===0?'משתמש':'קבוצה'} value={value} onChange={e=>setValue(e.target.value)}>{options.map(item=><MenuItem key={item.id} value={item.id}>{'display_name' in item?item.display_name:item.name}</MenuItem>)}</TextField><Button variant="contained" disabled={!value} onClick={add}>הוספה</Button></Stack></Paper></Stack>;
}
